    ...
    <>
      <Link className="relative button button--secondary button--lg m-2 block lg:inline-block" to="https://rdo.instatus.com">
        ...
        <span class="flex absolute h-4 w-4 top-0 right-0 -mt-2 -mr-2">
          <span class="animate-ping absolute inline-flex h-full w-full rounded-full opacity-75" style={{ backgroundColor: color }}></span>
          <span class="relative inline-flex rounded-full h-4 w-4" style={{ backgroundColor: color }}></span>
        </span>
      </Link>
    </>
  )
